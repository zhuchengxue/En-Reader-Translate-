...
5. 配置构建：
   ...
   - 构建命令：**留空**。
   - 构建输出目录：**留空**。
   - **根目录（Root directory，点开“高级”才看得到）：留空或填 `.`（英文句点，表示仓库根目录）** ← 关键：本仓库根目录就是应用根（`index.html` / `js` / `css` / `functions` 直接在同层），**没有** `英文网页阅读器` 子目录。若填成 `英文网页阅读器`，构建找不到文件、自动部署会静默失败，线上一直是旧版（表现为“怎么改都没用”）。
