...
隐私优先、纯前端、零服务器成本的英文原版书阅读器，对标微信读书的学英语体验。支持 TXT / EPUB / PDF 三格式，单击查词、双击整句翻译、书内全文搜索、排版设置、离线可用。所有数据只存在你自己的浏览器里。
...
书籍、笔记、生词、设置只存在于你的浏览器，不上传任何服务器。翻译 / 查词仅调用公共 API，可自建代理避免跨域与限流。

---

当前版本 `2026-07-28.8`。若遇按钮无响应，点书架「工具」看前端版本是否一致；不一致即浏览器在跑旧缓存，硬刷新 Ctrl+Shift+R 即可。
